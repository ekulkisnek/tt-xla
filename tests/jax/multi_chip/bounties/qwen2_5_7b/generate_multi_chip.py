...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen2.5-7B-Instruct JAX Inference with Custom Prompts"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prompt",
        type=str,
        default="Question: Sam scores 80 on the first test and 90 on the second. What score does he need on the third test to have an average of 85?",
        help="Custom prompt to generate text for",
    )
    parser.add_argument(
        "--max_tokens",
        type=int,
    ...
    parser.add_argument(
        "--no_realtime", action="store_true", help="Disable real-time text display"
    )
    parser.add_argument(
    ...
    output, peak_mem, avg_time_per_token = generate_text(
        model, params, tokenizer, args.max_tokens, args.prompt, show_realtime
    )
