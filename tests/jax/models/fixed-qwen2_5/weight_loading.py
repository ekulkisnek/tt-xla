...
"""Weight loading utilities for Qwen2.5 model."""
...
def get_mesh_and_partition_specs(mesh_shape: Tuple[int, int]):
    """
    Create device mesh and get partition specifications.
    
    Args:
        mesh_shape: Tuple of (batch_dim, model_dim) for the mesh
        
    Returns:
        Tuple of (Mesh, Dict) containing the device mesh and partition specs
    """
    # Create device mesh
    devices = jax.devices()
    ...
    if len(devices) < total_devices:
        raise ValueError(f"Not enough devices ({len(devices)}) for mesh shape {mesh_shape}. "
                       f"Required: {total_devices}")
    
    # Create mesh with named axes
    devices_array = np.array(devices[:total_devices]).reshape(mesh_shape)
    mesh = jax.sharding.Mesh(devices_array, ('batch', 'model'))
    
    # Define partition specs for different parameter types
    partition_specs = {
        # Embeddings
        ...
        'model/layers/.*/post_attention_layernorm/weight': P(None),
    ...
    return mesh, partition_specs

def apply_partition_specs(params: Dict, partition_specs: Dict, mesh: jax.sharding.Mesh) -> Dict:
    """
    Apply partition specifications to parameters.
    
    Args:
        params: Parameter dictionary
        partition_specs: Dictionary mapping parameter patterns to partition specs
        mesh: Device mesh
        
    Returns:
        Dictionary with sharded parameters
    """
    import re
    
    # Flatten params for easier matching
    flat_params = flatten_dict(params)
    sharded_params = {}
    
    # Apply partitioning to each parameter
...
def load_qwen_model(
    ...
    """
    Load Qwen2.5 model with tensor parallelism.
    ...
    if len(devices) < total_devices_needed:
        raise RuntimeError(
            f"Not enough devices ({len(devices)}) for mesh shape {mesh_shape}. "
            f"Need {total_devices_needed} devices. "
            "Please adjust mesh_shape or ensure enough devices are available."
        )
    
    try:
        # Create device mesh for tensor parallelism using the function from tensor_parallel.py
        mesh = create_device_mesh(mesh_shape)
    except Exception as e:
        raise RuntimeError(
            f"Failed to create device mesh with shape {mesh_shape}. "
            f"Error: {str(e)}. Please verify your mesh configuration and device setup."
        ) from e
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to load model configuration from {model_path}. "
            f"Error: {str(e)}. Please verify the model path and file permissions."
        ) from e

    # Define partition specs for model parameters
    param_specs = get_mesh_and_partition_specs(mesh_shape)[1]
    ...
    with mesh:
        try:
            ...
            # Load weights with appropriate error handling
            ...
            return model.bind(params), tokenizer
            
        except Exception as e:
            raise RuntimeError(
                f"Failed to initialize model or load weights. Error: {str(e)}. "
                "Please verify model compatibility and memory requirements."
            ) from e
...
def load_qwen_weights(
    ...
    try:
        ...
        if mesh is not None:
            partition_specs = get_mesh_and_partition_specs(mesh_shape)[1]
        else:
