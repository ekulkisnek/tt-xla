...
QWEN25_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`Qwen25Config`]): Model configuration class with all the parameters of the model.
...
class FlaxQwen25Attention(nn.Module):
    ...
    def setup(self):
        ...
        dense = partial(
            nn.Dense,
            use_bias=False,  # bias is not used in Qwen2/2.5
            dtype=self.dtype,
...
class FlaxQwen25LayerCollection(nn.Module):
    ...
    def setup(self):
        self.blocks = [
            FlaxQwen25DecoderLayer(self.config, dtype=self.dtype, name=str(i))
    ...
    def __call__(
        ...
        for block in self.blocks:
            ...
            use_sliding_window = (
                ...
                and int(block.name) < self.config.max_window_layers
            )
        ...
        # this contains possible `None` values - `FlaxQwen25Module` will filter them out
...
class FlaxQwen25PreTrainedModel(FlaxPreTrainedModel, FlaxQwen25WithSharding):
    ...
    @add_start_docstrings_to_model_forward(QWEN25_INPUTS_DOCSTRING)
    def __call__(
        self,
        input_ids,
        attention_mask=None,
        ...
        # if past_key_values are passed then cache is already initialized a private flag init_cache has to be passed down to ensure cache is used. It has to be made sure that cache is marked as mutable so that it can be changed by FlaxQwen25Attention module
        if past_key_values:
...
@add_start_docstrings(
    """
    The Qwen2.5 Model transformer with a language modeling head (linear layer) on top.
    """,
    QWEN25_START_DOCSTRING,
)
class FlaxQwen25ForCausalLM(FlaxQwen25PreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, max_length, attention_mask: Optional[jax.Array] = None):
        ...
        # But since Qwen2.5 uses a causal mask, those positions are masked anyways.
...
__all__ = ["FlaxQwen25ForCausalLM", "FlaxQwen25Model", "FlaxQwen25PreTrainedModel"] 